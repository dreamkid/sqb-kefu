...
// 请求的域名，基本地址
axios.defaults.baseURL = store.state.url ? `http://${store.state.url}:13086` : ''
// 环境的切换
// if (process.env.NODE_ENV === 'development') {
//   axios.defaults.baseURL = baseUrlDev || 'http://192.168.0.115:13086'
...
httpInstance.interceptors.request.use(
  (config) => {
    if (['/wechat/v1/wecatstatus', '/pc/account-status-change'].includes(config.url)) {
      config.baseURL = process.env.VUE_APP_ANOTHER_URL
    }
    if (!config.baseURL) config.baseURL = `http://${store.state.url}:13086`
    // 在发送请求之前做些什么
    // if (process.env.NODE_ENV === "development") {
...
httpInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response && error.response.status && error.data) {
      errorHandle(error.response.status, error.data.message)
